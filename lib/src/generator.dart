...
class Generator {
  ...
  List<List<int>> _toColumnFormat(Image imgSrc, int lineHeight) {
    ...
    while (left < widthPx) {
      final Image slice = copyCrop(
        biggerImage, x: left, y: 0, width: lineHeight, height: heightPx,
      );
      final Uint8List bytes = slice.getBytes(); // slice.getBytes(format: Format.luminance);
      blobs.add(bytes);
      left += lineHeight;
    }
  ...
  List<int> _toRasterFormat(Image imgSrc) {
    ...
    final List<int> buffer = image.getBytes(order: ChannelOrder.rgba,); // image.getBytes(format: Format.rgba);
